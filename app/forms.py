...
class ClienteForm(FlaskForm):
    ...
    latitud = DecimalField('Latitud (Google Maps)', places=7, validators=[Optional()])
    longitud = DecimalField('Longitud (Google Maps)', places=7, validators=[Optional()])
    foto = TextAreaField('URL Foto del Cliente', validators=[Optional(), Length(max=500)])
...
class PrestamoForm(FlaskForm):
    ...
    tipo_garantia = SelectField('Tipo de Garantía',
                                 choices=[('personal', 'Personal'), ('prendaria', 'Prendaria'),
                                          ('hipotecaria', 'Hipoteca')])
    metodo_amortizacion = SelectField('Método Amortización',
                                       choices=[('frances', 'Francés (Cuota Fija)'),
                                                ('alemán', 'Alemán (Amortización Constante)'),
...
class TasaInteresForm(FlaskForm):
    ...
    valor = DecimalField('Valor (decimal, ej: 0.03 = 3%)', places=4, validators=[DataRequired(), NumberRange(min=0.0001, max=99.9999)])
